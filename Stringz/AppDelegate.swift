...
extension AppDelegate {
    @objc func performOpen(_ sender: Any) {
        ...
        dialog.allowedFileTypes = ["xcodeproj", "csv"]
            ...
            if result == NSApplication.ModalResponse.OK {
                if let url = dialog.url {
                    if url.pathExtension == "xcodeproj" {
                        let _ = self.openProject(url)
                    }
                    if url.pathExtension == "csv", let stream = InputStream(url: url) {
                        self.parseCSVFile(stream: stream)
                    }
                } else {
...
extension AppDelegate {
    ...
    func parseCSVFile(stream: InputStream)  {
        ...
        do {
            ...
            let savePath = try Path.uniqueTemporary() + Path("localized")
            var paths = [Path]()
            ...
            for title in header { //
                ...
                let directoryPath = savePath + Path(fileName)
                if !directoryPath.exists {
                    try directoryPath.mkpath()
                }
                let localizablePath = directoryPath + Path("Localizable.strings")
                ...
                for (index,title) in header.enumerated() { //
                    ...
                    if path.exists {
                        if let fileHandle = try? FileHandle(forWritingTo: path.url) {
                            ...
                            fileHandle.write("\n".data(using: .utf8)!) // 添加换行符
                            fileHandle.closeFile()
                        }
                    }
                    else {
                        try result.write(to: path.url, atomically: true, encoding: .utf8)
                        try "\n".write(to: path.url, atomically: true, encoding: .utf8)
                    }
            ...
            DispatchQueue.main.async {
                NSWorkspace.shared.open(savePath.url)
            }
        } catch {
            debugPrint(error)
        }
